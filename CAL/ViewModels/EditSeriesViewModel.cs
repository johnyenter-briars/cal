...
namespace CAL.ViewModels
{
    ...
    [QueryProperty(nameof(YearlyEvent), nameof(YearlyEvent))]
    ...
    public class EditSeriesViewModel : BaseViewModel
    {
        ...
        private bool yearlyEvent;
        public bool YearlyEvent
        {
            get {
                if (yearlyEvent) return true;
                if (id != Guid.Empty && !repeatOnSun && !repeatOnMon && !repeatOnTues && !repeatOnWed && !repeatOnThurs && !repeatOnFri && !repeatOnSat && repeatEveryWeek == 0)
                {
                    yearlyEvent = true;
                    return true;
                }
                ...
                return false;
                //yearlyEvent || (Id != null && !repeatOnSun && !repeatOnMon && !repeatOnTues && !repeatOnWed && !repeatOnThurs && !repeatOnFri && !repeatOnSat && repeatEveryWeek == 0);
            }
            set => SetProperty(ref yearlyEvent, value);
        }
        ...
        private async void OnSave()
        {
            ...
            if (yearlyEvent)
            {
                var years = PreferencesManager.GetDefaultYearsToRepeat();
